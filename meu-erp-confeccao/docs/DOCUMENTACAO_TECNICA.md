...
- **Acesso Master Flexível:** Usuários com a role `SUPERADMIN` (da plataforma SaaS) podem realizar login diretamente nas URLs dos tenants (ex: `cliente.localhost`). O sistema reconhece a role de super usuário e permite a entrada, repassando o contexto para o esquema (`schema`) adequado do lojista de forma transparente, através do `TenantInterceptor`.
